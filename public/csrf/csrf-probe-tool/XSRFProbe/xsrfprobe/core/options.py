...
optional.add_argument(
    "--client-url",
    help="URL of simulated client page to auto-fetch session cookies and CSRF token via CORS",
    dest="client_url",
    type=str,
    default=""
)
...
if config.SITE_URL:
    # Determine the directory name for output
    try:
        direc = tld.get_fld(config.SITE_URL)
    except tld.exceptions.TldDomainNotFound:
        # Fallback to 'localhost' if TLD extraction fails
        direc = 'localhost'
        # Check if the URL is an IP address
        ip_match = re.search(IP, config.SITE_URL)
        if ip_match:
            direc = ip_match.group(0)

    # Set output directory based on --output argument or default
    if args.output:
        output_path = f"{args.output}{direc}"
    else:
        output_path = f"xsrfprobe-output/{direc}"
    ...
    config.OUTPUT_DIR = f"{output_path}/"
