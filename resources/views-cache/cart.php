...
<head>
  ...
  <link rel="stylesheet" href="<?php echo htmlspecialchars(strval(assetsUrl('css/cart.css'))); ?>">
  <script defer src="<?php echo htmlspecialchars(strval(assetsUrl('/js/scripts.js'))); ?>"></script>
  <script defer src="<?php echo htmlspecialchars(strval(assetsUrl('/js/cart.js'))); ?>"></script>
</head>
<body>
  <header>
    <div class="logo">
            ...
            <div class="cart-container">
              ...
              <div class="cart-prod-detail">
                <div class="cart-prod-name">
                  <h2 id="product-name"><?php echo htmlspecialchars(strval($product->name)); ?></h2>
                </div>
                <div class="cart-info">
              ...
              <div class="remove-button">
                <button type="button" data-id="<?php echo htmlspecialchars(strval($product->id)); ?>">x</button>
              </div>
            </div>
          <?php $index++; $count++; $even = !$even; $odd = !$odd; endforeach; ?>
          <div class="cart-button">
            <button type="button" id="delete-cart-button">Delete Cart</button>
            <a href="<?php echo htmlspecialchars(strval(url('carts/user-cart/checkout'))); ?>"><button>Proceed</button></a>
          </div>
        <?php endif; ?>
      </div>
